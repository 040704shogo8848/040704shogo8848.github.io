...
第四に、任期の設計である。DARPAはプログラムマネージャーの在任が期限付きであることを採用ページで示唆しているが、具体的な年数は公式ページから確認できていない[出典](https://www.darpa.mil/careers)。3年から5年という数字が一般に語られるものの、一次資料で確認できていないため本章では確定させない。VCのパートナーはファンドの10年満期に縛られるが、個人の任期はない。
...
組織を作れば解決するという理解も成り立たない。日本は2024年10月1日に防衛イノベーション科学技術研究所を約100人態勢で設置し、DARPAと国防イノベーションユニットを参考にしている[出典](https://ja.wikipedia.org/wiki/防衛イノベーション科学技術研究所)。参照したのは研究の仕組みであり、単価を決める原価計算方式と調達数量には手が付いていない。防衛装備庁は迎撃ドローンの早期取得プログラムのページを設けているが、本文がHTTP 403で取得できず、調達数量と価格帯と公募条件は確認できていない。ここが分かれば、日本が想定する単価と数量が判明し、障害の大きさを数値で示せる。
...
- DARPAは株式を取らず契約と助成で資金を出し、市場規模と競合優位を審査基準に置かない。既存大手に資金を固定しない設計が、日本の組合型と逆の産業構造を生んだ
...
- 日本の1980年代の家計貯蓄率の公式時系列を内閣府と総務省統計局から直接取得できなかった。本章の貯蓄比較は国内総貯蓄の対GDP比で代替している
- 日本の半導体世界シェアのピークについて、1986年46%とする記述と1988年50.3%とする記述が並存する。同一系列での比較ができていない
...
- DARPAのプログラムマネージャーの任期年数は公式ページから確認できていない。3年から5年という数字は一般に語られるが未確定である
...
- US Steelの1901年の証券水増しが、その後どの規模の株価下落として実現したかは一次資料に当たれていない
- 熱風炉によるコスト低下が製鉄株のバブルを抑えたという説明を裏づける同時代の株価データは取得できていない
- 脱炭素方針が新規原料炭鉱山への投資を細らせているという説明の定量的裏づけ、黒鉛電極とニードルコークスの価格水準と変動幅、日本の高炉各社の原料炭輸入依存率は、いずれも未確認である
