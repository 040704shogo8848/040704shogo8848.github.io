...
- SK hynixとSamsungの投資対象がどこで分岐したかを、両社の設備投資の年次配分で追えていない。HBMと汎用DRAMへの配分比の時系列が要る。
- エージェンティックAI向けデータセンターチップでRISC-Vが有利になる根拠を確認できていない。命令セットの差が推論ワークロードの効率に効く経路が、ベンチマークで示されているかは未確認である。
## 出典
