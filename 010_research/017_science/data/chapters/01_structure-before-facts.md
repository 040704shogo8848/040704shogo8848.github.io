...
ここで注意が要る。一般化は自動的に計算を速くしない。n次のフル行列に対する掃き出し法の演算回数は、乗算と減算がそれぞれ2nの3乗足す3nの2乗引く5nを6で割った回数である[出典](https://en.wikipedia.org/wiki/Gaussian_elimination)。LAPACK も1右辺の線形求解の基準演算数を0.67掛けるNの3乗として扱う[出典](https://www.netlib.org/lapack/lug/node71.html)。この式に rank は入らない。rank が下がっても、密行列を素直に解く限り手順は短くならない。
...
素粒子物理の多重項が最も近い。ゲルマンとネエマンのエイトフォールドウェイは、アップとダウンとストレンジの3種類のクォークを入れ替えるフレーバーSU3対称性で粒子を分類した。バリオン十重項に空席が1つ残り、ゲルマンは1962年、ストレンジネス-3、電荷-1、質量およそ1680 MeV/c2の粒子を予言した。1964年にブルックヘブン国立研究所でオメガマイナス粒子が確認された[出典](https://en.wikipedia.org/wiki/Eightfold_way_(physics))。==表の周期性を対称群の既約表現に置き換えただけで、手続きは周期表と同型である==。
...
線形代数を早い位置に置く設計の根拠は、歴史順の再現ではなく逆算である。根拠は3点ある。第1に、前提知識が少ない。第2に、線形独立や基底や次元は定義から数行で反例が作れるため、抽象的な定義に慣れる訓練の媒体に向く。第3に、下流科目の依存が広い。多変数微積分の全微分は線形写像そのものであり、定数係数の線形常微分方程式系は固有値問題に帰着し、数値計算と統計と機械学習は行列演算を基盤にする。依存関係のグラフで入次数が小さく出次数が大きい科目を前に置くのは、カリキュラム設計の一般原則である。
...
高校で微積を先に置く順序は妥当である。ただし理由は、実数のイメージが湧くからではない。高校の数学IIIは極限、微分法、積分法の3領域で構成され、極限はイプシロン・デルタ論法ではなく値の近づき方の直感的な把握として扱われる[出典](https://ja.wikipedia.org/wiki/%E6%95%B0%E5%AD%A6III)。実数の構成も連続性の公理も出てこない。実数論を回避しているから高校で成立している。妥当性の第2の理由は動機の可視性である。速度、加速度、面積、体積、最大最小は物理と同時に扱える題材であり、微積の誕生自体が力学と天文学の要求から来ている[出典](https://en.wikipedia.org/wiki/History_of_calculus)。
...
- 1869年時点の既知元素数が56個か63個かは確定していない。メンデレーエフの1869年論文を取得できていない。
- エカケイ素の予測原子量について72とする記述と72.64とする記述の差が、後世の丸めか転記かは確認できていない。1871年論文の原文表を取得できていない。
- 質量獲得機構に対応する粒子の測定質量は、参照したCERNの解説ページに記載がなく取得できていない。
- 日本の大学理系学部における微分積分学と線型代数学の配当学期の実態は、一次資料で確認できていない。
- 高校学習指導要領の年度と単元名は日本語版ウィキペディア経由であり、文部科学省の解説PDFで確認できていない。
- Dorier と Sierpinska による obstacle of formalism の内容は本文を取得できておらず、未検証のままである。
- 高次元で最近傍と最遠点の距離比が1へ近づく現象の定量的な出典は取得できていない。
- ジョルダン標準形の数値的不安定性について、Golub と Wilkinson の1976年の論文本文は取得できておらず、記述は二次資料に依拠している。
